<script lang="ts">
	...
	import { setActingClient } from '$lib/api/acting';
	import {
		protocolsApi,
		COMPOUND_CLASSES,
		FREQUENCIES,
	...
	// `acting` scopes every protocolsApi call below to a client (coach editing); null
	// for the owner's own builder. Set before load() runs (script init precedes onMount).
	let { protocolId, acting = null, backHref = '/protocols/manage' }: {
		protocolId: number;
		acting?: number | null;
		backHref?: string;
	} = $props();
	setActingClient(acting);
	onDestroy(() => setActingClient(null));
...
{:else if protocol}
	<div class="flex items-center justify-between">
		<div>
			<a class="text-sm text-neutral-400 hover:text-white" href={backHref}>← Back</a>
